...
function ExamScreen({
  ...
  /* =====================================================
     CORRECTED EXAM START TIME

     The old code incorrectly created the start time when
     the student submitted the exam. This version records
     the real time the exam screen is entered.

     sessionStorage preserves the start time if the student
     refreshes the page during the same attempt.
     ===================================================== */

  const startTimeKey =
    `cbtStartTime:${studentId}:${exam.id}`;

  const [startTime] = useState(() => {
    const saved =
      sessionStorage.getItem(startTimeKey);
  ...
  async function submitExam(
    ...
    try {
          ...
          "/api/cbt-submit",
          {
            ...
            body:
              JSON.stringify({
                ...
                /* IMPORTANT:
                   Use the actual time the exam began,
      ...
      /* Clear the stored start time only after the
         server confirms that the attempt was saved. */
